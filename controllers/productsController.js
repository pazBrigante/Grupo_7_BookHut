...
const controller = {
	...
	update: function (req, res) {
		...
		if (errors.isEmpty()) {
			...
			db.Catalogo.findByPk(req.params.id)
				.then(resultado => {
					res.render("./productos/product-edit-form.ejs", {
						"productEdit": resultado, "usuarioActual": req.session.usuarioLogueado
	...
	search: (req, res) => {
		...
		db.Catalogo.findAll({
			...
			.then(resultado => {
				res.render("./productos/resultadoBusqueda", {
					"orden": orden,
					"textoBusqueda": textoBusqueda, "resultado": resultado, "catalogo": resultado,
					"id": req.params.id, "usuarioActual": req.session.usuarioLogueado
				});
			})
			.catch(error => {
				res.redirect("views/partials/not-found.ejs")
